...
def solve(path_to_problem, action):
    global args
    sepa_list = [
        # 'closecuts',
        'disjunctive',
...
def multiprocess_context_helper(helper_args):
    process_num, Bandit, val_flag = helper_args
    # sample problem
    ...
    input_context = _context.getInputContextAtStep(None, path_to_problem, 1, args.instances)
...
def multiprocess_solve_helper(helper_args):
    subprocess_num, process_num, action, path_to_problem = helper_args
    print(f"process {process_num}-{subprocess_num}: solving the problem with the chosen arm...")
    improv, reward = solve(path_to_problem, action)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ucb_val_on', type=int, default=0, help='whether to use ucb during validation')
    parser.add_argument('--actionsdir', type=str, default='./restricted_space', help='the directory to save the restricted action space')
    ...
    path_to_logdir = args.logdir + f"/{args.instances}_k1"
    if not os.path.exists(path_to_logdir):
        os.makedirs(path_to_logdir)
    writer = SummaryWriter(path_to_logdir)
    writer_train_len = 0
    ...
    for t in range(bandit_epoch):
        ...
        # save the model
        os.makedirs(args.savedir + f"/{args.instances}_k1", exist_ok=True)
        model_path = args.savedir + f"/{args.instances}_k1/model-{t}"
        Z_path = args.savedir + f"/{args.instances}_k1/Z-{t}"
